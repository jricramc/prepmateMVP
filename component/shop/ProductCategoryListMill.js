...
import { allProductListMill } from '@/data/Data';
            ...
            <ul className="product-categories">
                {categories.map(categoryObj => (
                    <li key={categoryObj.name} onClick={() => handleCategoryClick(categoryObj.name)}
                        className={activeCategory === categoryObj.name ? 'active' : ''}>
                        {categoryObj.label} ({categoryObj.name === null ? allProductListMill.length :
                            allProductListMill.filter(product => product.category === categoryObj.name).length})
                        <ul>
                            {allProductListMill.filter(product => product.category === categoryObj.name)
                                .map(product => (
                                    <li key={product.id}>
...
export default ProductCategoryListMill;
