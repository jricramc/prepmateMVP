...
import { allProductListSilk } from '@/data/Data';
            ...
            <ul className="product-categories">
                {categories.map(categoryObj => (
                    <li key={categoryObj.name} onClick={() => handleCategoryClick(categoryObj.name)}
                        ...
                        {categoryObj.label} ({categoryObj.name === null ? allProductListSilk.length :
                            allProductListSilk.filter(product => product.category === categoryObj.name).length})
                        <ul>
                            {allProductListSilk.filter(product => product.category === categoryObj.name)
                                .map(product => (
                                    <li key={product.id}>
...
export default ProductCategoryListSilk;
